...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MAP_PIN_TO_RC(x)  ((x) == GPIO_PIN_SET ? 2000 : 1000)
#define ADC_MIN 400
#define ADC_MAX 3600
#define DEADBAND 15
/* USER CODE END PD */
...
int main(void)
{
  ...
  MX_TIM3_Init();
  /* USER CODE BEGIN 2 */
	HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc_joystick, 4);
  ...
  while (1)
  {
		...
		if (HAL_GetTick() - last_heartbeat >= 1000)
    {
		...
		if (HAL_GetTick() - last_att >= 100)
		{
			last_att = HAL_GetTick();
			roll += 0.01f;
