...
#include <hyp.msgs.BatteryStatus.h>
...
uint32_t hyp_msgs_BatteryStatus_encode(struct hyp_msgs_BatteryStatus* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, HYP_MSGS_BATTERYSTATUS_MAX_SIZE);
    _hyp_msgs_BatteryStatus_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > HYP_MSGS_BATTERYSTATUS_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (_hyp_msgs_BatteryStatus_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct hyp_msgs_BatteryStatus sample_hyp_msgs_BatteryStatus_msg(void) {
    ...
    struct hyp_msgs_BatteryStatus msg;
